...
class Play(Movement):
    ...
    def __init__(self, main_info_model, tile_detector_model, window_controller):
        ...
        self.scene_data = []
        self.gamemode = bot_config["gamemode"]
        self.should_detect_walls = bot_config["gamemode"] in ["brawlball", "brawl_ball", "brawll ball", "showdown", "aggressive_showdown"]
        ...
        general_cfg = load_toml_as_dict("cfg/general_config.toml")
        self.show_debug_window = str(general_cfg.get("show_debug_window", "no")).lower() in ("yes", "true", "1")
        self._debug_window_name = "Pyla Debug"
    ...
    def loop(self, brawler, data, current_time):
        ...
        elif self.gamemode == "aggressive_showdown":
            movement = self.get_aggressive_showdown_movement(
                player_data=data['player'][0],
                enemy_data=data['enemy'],
                teammate_data=data.get('teammate'),
                walls=data['wall'],
    ...
    def draw_debug_overlay(self, frame, data, brawler, movement=None):
        """Live visualization of detections. Enable via general_config.show_debug_window = "yes"."""
        ...
        try:
            ...
            if data:
                draw_boxes(data.get('wall') or [], (160, 160, 160), 'wall')
                draw_boxes(data.get('teammate') or [], (0, 200, 0), 'ally')
                draw_boxes(data.get('enemy') or [], (0, 0, 255), 'enemy')
                player_boxes = data.get('player') or []
                if player_boxes:
                    draw_boxes([player_boxes[0]], (255, 140, 0), 'player')
                    cx, cy = self.get_player_pos(player_boxes[0])
                    cx, cy = int(cx), int(cy)
                    try:
                        safe_range, attack_range, super_range = self.get_brawler_range(brawler)
                        cv2.circle(overlay, (cx, cy), int(safe_range), (0, 255, 255), 1)
                        cv2.circle(overlay, (cx, cy), int(attack_range), (0, 0, 255), 1)
                        cv2.circle(overlay, (cx, cy), int(super_range), (255, 0, 255), 1)
                    except Exception:
                        pass

            status_lines = [
                f"brawler: {brawler}  mode: {self.gamemode}",
                f"super: {self.is_super_ready}  gadget: {self.is_gadget_ready}  hyper: {self.is_hypercharge_ready}",
                f"move: {movement}",
            ]
            for i, line in enumerate(status_lines):
                cv2.putText(overlay, line, (12, 24 + i * 22),
            ...
            if not self._debug_window_inited:
                cv2.namedWindow(self._debug_window_name, cv2.WINDOW_NORMAL)
                self._debug_window_inited = True
            cv2.imshow(self._debug_window_name, overlay)
            cv2.waitKey(1)
        except Exception as e:
            print(f"[debug overlay] error: {e}")

    def get_closest_teammate(self, teammate_data, player_coords, walls):
        if not teammate_data:
            return None, None
        closest_pos = None
        closest_distance = float('inf')
        for teammate in teammate_data:
            tm_pos = self.get_enemy_pos(teammate)
            distance = self.get_distance(tm_pos, player_coords)
    ...
    def get_showdown_movement(self, player_data, enemy_data, teammate_data, walls, brawler):
        """
        Showdown logic:
          - Follow the closest ally the whole time.
          - If an enemy is within attack range, attack (and super/gadget/hypercharge as usual).
        """
        ...
        # Release held attack after max hold duration (same as standard logic)
        ...
        if debug: print("[showdown] player pos:", player_pos)

        # --- Attack logic: hit any enemy in range ---
        enemy_coords, enemy_distance = (None, None)
        if self.is_there_enemy(enemy_data):
            enemy_coords, enemy_distance = self.find_closest_enemy(enemy_data, player_pos, walls, "attack")

        if enemy_coords is not None and enemy_distance is not None:
            # Super (+ hypercharge) if available and enemy is in super range
            ...
            # Normal attack if in attack range and hittable
        ...
        # --- Movement logic: follow the closest ally ---
        teammate_pos, teammate_distance = self.get_closest_teammate(teammate_data, player_pos, walls)

        if teammate_pos is not None:
            # Stay within safe_range of the teammate.
            follow_distance = max(safe_range, 1)
            direction_x = teammate_pos[0] - player_pos[0]
            direction_y = teammate_pos[1] - player_pos[1]

            if teammate_distance > follow_distance:
                ...
                # Close enough: if an enemy is also close, kite away; else nudge along
                if enemy_coords is not None and enemy_distance is not None and enemy_distance <= safe_range:
                    edx = enemy_coords[0] - player_pos[0]
                    edy = enemy_coords[1] - player_pos[1]
                    move_horizontal = self.get_horizontal_move_key(edx, opposite=True)
                    move_vertical = self.get_vertical_move_key(edy, opposite=True)
                else:
                    move_horizontal = self.get_horizontal_move_key(direction_x)
                    move_vertical = self.get_vertical_move_key(direction_y)
        ...
        # No ally detected -> fall back to default exploration movement
        return self.no_enemy_movement(player_data, walls)

    def get_aggressive_showdown_movement(self, player_data, enemy_data, teammate_data, walls, brawler):
        """
        Aggressive Showdown logic:
          - Stay near teammates while no enemies are visible.
          - As soon as an enemy appears, rush toward them and attack.
        """
        brawler_info = self.brawlers_info.get(brawler)
        if not brawler_info:
            raise ValueError(f"Brawler '{brawler}' not found in brawlers info.")
        ...
        # Release held attack after max hold duration
        if (must_brawler_hold_attack
                and self.time_since_holding_attack is not None
                and time.time() - self.time_since_holding_attack
                >= brawler_info['hold_attack'] + self.seconds_to_hold_attack_after_reaching_max):
        ...
        safe_range, attack_range, super_range = self.get_brawler_range(brawler)
        player_pos = self.get_player_pos(player_data)
        if debug: print("[aggressive_sd] player pos:", player_pos)
        ...
        if enemy_coords is not None and enemy_distance is not None:
            # ---- Enemy visible: rush toward it and attack ----
            if debug: print("[aggressive_sd] rushing enemy at", enemy_coords, "dist", enemy_distance)

            # Super (+ hypercharge) when possible
            if self.is_super_ready and self.time_since_holding_attack is None:
                super_type = brawler_info['super_type']
                enemy_hittable_super = self.is_enemy_hittable(player_pos, enemy_coords, walls, "super")
                if (enemy_hittable_super and
            ...
            # Always push toward the enemy (aggressive)
        ...
        # ---- No enemy visible: stick with teammates ----
        teammate_pos, teammate_distance = self.get_closest_teammate(teammate_data, player_pos, walls)
        ...
        # No ally, no enemy -> default exploration
        return self.no_enemy_movement(player_data, walls)
    ...
    def main(self, frame, brawler, main):
        ...
        raw_data = data  # keep a reference for the debug overlay even if validation fails
        ...
        if not data:
            ...
            if current_time - self.time_since_last_proceeding > self.no_detection_proceed_delay:
                ...
                if current_state != "match":
                    ...
                    self.time_since_last_proceeding = time.time()
            self.draw_debug_overlay(frame, raw_data, brawler, movement=None)
            return
        self.time_since_last_proceeding = time.time()
        self.is_hypercharge_ready = False
        ...
        self.draw_debug_overlay(frame, data, brawler, movement=movement)
