...
class Hub:
    ...
    def _init_overview_tab(self):
        ...
        self.rb_aggressive_sd_3 = create_gamemode_button(
            self.gm3_frame, "aggressive_showdown", "Aggressive SD", orientation=3
        )
        ...
        self.rb_aggressive_sd_3.grid(row=0, column=2, padx=S(10), pady=S(5))
        self.rb_other_3.grid(row=0, column=3, padx=S(10), pady=S(5))
        ...
        def refresh_gm_buttons():
            ...
            set_button_color(self.rb_aggressive_sd_3, "aggressive_showdown")
    ...
    def _init_additional_tab(self):
        ...
        # Show Debug Window toggle (general_config)
        lbl_debug_win = ctk.CTkLabel(container, text="Show Debug Window:", font=("Arial", S(18)))
        lbl_debug_win.grid(row=row_idx, column=0, sticky="e", padx=S(20), pady=S(10))
        self.general_config.setdefault("show_debug_window", "no")
        debug_win_var = tk.BooleanVar(
            value=(str(self.general_config["show_debug_window"]).lower() in ["yes", "true"])
        )

        def toggle_debug_window():
            self.general_config["show_debug_window"] = "yes" if debug_win_var.get() else "no"
            save_dict_as_toml(self.general_config, self.general_config_path)

        debug_win_cb = ctk.CTkCheckBox(
            container,
            text="",
            variable=debug_win_var,
        ...
        debug_win_cb.grid(row=row_idx, column=1, sticky="w", padx=S(20), pady=S(10))
        self.attach_tooltip(debug_win_cb,
            "Opens a live window showing what the AI is detecting:\n"
            "  - player (orange), allies (green), enemies (red), walls (gray)\n"
            "  - safe / attack / super range circles around the player\n"
            "  - current brawler, gamemode, movement, super/gadget/hyper status")
        row_idx += 1
